...
/**
 * ✅ Modelo para progreso (lo manda el backend por SignalR cada N facturas)
 * La idea: mandar SOLO resumen (números), no el JSON completo.
 */
export type AccionRips = 'GENERAR' | 'PRESENTAR';

export class ProgresoRipsModel {
  accion: AccionRips = 'PRESENTAR';

  total: number = 0; // cuántos documentos en total
  procesadas: number = 0; // cuántos ya se procesaron
  exitosas: number = 0; // cuántas salieron OK
  fallidas: number = 0; // cuántas salieron mal

  ultimoDocumento?: string; // ej: RYDT123 o numFactura
  mensaje?: string; // ej: "Enviando..." / "Generando XML..." etc.

  // opcional: tiempos (si luego los quieres)
  startedAtIso?: string;
}
