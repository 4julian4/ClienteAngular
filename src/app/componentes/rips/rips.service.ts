...
export class RipsService {
  ...
  @Output() respuestaGenerarRdaEmit: EventEmitter<boolean> =
    new EventEmitter<boolean>();
  ...
  async startConnectionGenerarRdaDesdeRipsExistente(
    sedeId: number,
    payloadJson: string,
  ): Promise<void> {
    try {
      ...
      if (this.onRespuestaGenerarRda) {
        this.signalRService.off(
          'RespuestaGenerarRdaDesdeRipsExistente',
          this.onRespuestaGenerarRda,
        );
      }
      ...
      this.onRespuestaGenerarRda = (
        ...
        try {
          ...
          this.respuestaGenerarRdaEmit.emit(!!ok);
        } finally {
          this.respuestaPinService.updateisLoading(false);
      ...
      await this.signalRService.invoke(
        'GenerarRdaDesdeRipsExistente',
        sedeId,
        payloadJson,
      );
    } catch (err) {
      console.error('Error al generar RDA desde RIPS existente:', err);
      this.respuestaPinService.updateisLoading(false);
    }
  }
