...
import { AgregarDatosPersonalesService } from './agregar-datos-personales.service';
import { MensajesUsuariosService } from '../mensajes-usuarios';
import { Router } from '@angular/router';
import { Subject, takeUntil } from 'rxjs';

@Component({
  selector: 'app-agregar-datos-personales',
  templateUrl: './agregar-datos-personales.component.html',
  styleUrl: './agregar-datos-personales.component.scss'
})
export class AgregarDatosPersonalesComponent implements OnInit {
  @ViewChild('inputCamara', { static: false }) inputCamara!: ElementRef;
  ...
  idAnamnesisParaGuardarEditar: number = 0;
  private destruir$: Subject<boolean> = new Subject<boolean>();
  //fotoFrontalBase64: string = ''; // Aquí se almacena la imagen en base64
  ...
  constructor(
    ...
    private agregarDatosPersonalesService: AgregarDatosPersonalesService,
  ...
  async ngOnInit(): Promise<void> {
    ...
    this.respuestaPinService.sharedcambiarDoctorSeleccionadoData.pipe(takeUntil(this.destruir$)).subscribe(data => {
      if (data != null) {
        this.doctorGuardar = data;
      }
    });

    this.inicializarFormulario();
    this.formularioDatosPersonales.valueChanges.pipe(takeUntil(this.destruir$)).subscribe(valores => {
      Object.keys(valores).forEach(campo => {
        if (typeof valores[campo] === 'string') {
          // Convertir a mayúsculas y actualizar el valor del campo
          this.formularioDatosPersonales.get(campo)?.setValue(valores[campo].toUpperCase(), { emitEvent: false });
        }
      });
    });

    this.respuestaPinService.shareddatosPersonalesParaEditarData.pipe(takeUntil(this.destruir$)).subscribe(data => {
      if (data) {
        console.log(data);
        // Modo edición: Cargar datos existentes en el formulario
        this.resultadoBusquedaDatosPersonalesCompletos = data.datosPersonales;
        this.respuestaPinService.updateNotaImportante(this.resultadoBusquedaDatosPersonalesCompletos.IMPORTANTE);
        this.fotoFrontalBase64 = data.strFotoFrontal || this.imagenPorDefecto;
        // Verifica si la hora está en formato Date y conviértela a "hh:mm AM/PM"
        //console.log(this.resultadoBusquedaDatosPersonalesCompletos.HORA_INGRESO);
        //const horaFormateada = this.formatHora(this.resultadoBusquedaDatosPersonalesCompletos.HORA_INGRESO);
        //cambiamos HORA_INGRESO a formato hh:mm AM/PM
        console.log(this.resultadoBusquedaDatosPersonalesCompletos.HORA_INGRESO);
        console.log(this.resultadoBusquedaDatosPersonalesCompletos.FECHA_INGRESO);
        const horaFormateada = this.convertirAHora24(this.resultadoBusquedaDatosPersonalesCompletos.HORA_INGRESO.toString());
        console.log(horaFormateada);
        this.formularioDatosPersonales.patchValue({
          ...this.resultadoBusquedaDatosPersonalesCompletos, HORA_INGRESO: horaFormateada
        });
      } else {
    ...
    // Suscribirse a cambios en los campos "NOMBRES" y "APELLIDOS"
    this.formularioDatosPersonales.get('NOMBRES')!.valueChanges.pipe(takeUntil(this.destruir$)).subscribe(() => this.actualizarNombreCompleto());
    this.formularioDatosPersonales.get('APELLIDOS')!.valueChanges.pipe(takeUntil(this.destruir$)).subscribe(() => this.actualizarNombreCompleto());
    ...
    this.respuestaPinService.shareddatosRespuestaPinData.pipe(takeUntil(this.destruir$)).subscribe(data => {
      if (data != null) {
    ...
    this.respuestaPinService.sharedSedeData.pipe(takeUntil(this.destruir$)).subscribe(data => {
      if (data != null) {
    ...
    //Datos Personales
    this.datosPersonalesService.respuestaDatosPersonalesEmit.pipe(takeUntil(this.destruir$)).subscribe(async (respuestaBusquedaDatosPersonales: RespuestaDatosPersonales) => {
  ...
  private formatHora(hora: any): string {
    if (!hora) return ''; // Si es null o undefined, devolver vacío

    let date: Date;

    if (hora instanceof Date) {
      date = hora; // Si ya es un Date, lo usamos directamente
    } else if (typeof hora === 'string') {
      const [horas, minutos, segundos] = hora.split(':').map(Number); // Convertimos "HH:mm:ss" en números
      if (isNaN(horas) || isNaN(minutos)) return ''; // Validamos que sean números
      date = new Date(); // Creamos una nueva fecha con la hora recibida
      date.setHours(horas, minutos, segundos || 0);
    } else {
      return ''; // Si no es un tipo válido, devolver vacío
    }

    if (isNaN(date.getTime())) return ''; // Si la fecha no es válida, devolver vacío
    ...
    const minutosStr = minutos < 10 ? '0' + minutos : minutos; // Formato 08 en vez de 8
  ...
  inicializarFormulario() {
    ...
    this.formularioDatosPersonales = this.formBuilder.group({
      ...
      ACTIVO: ['0'],
      IDREFERIDOPOR: ['0'],
      COD_DOCTOR: [this.idDoctorGuardar],
      DOCTOR: [this.doctorGuardar],
    });
  ...
  actualizarNombreCompleto(): void {
    const nombres = this.formularioDatosPersonales.get('NOMBRES')!.value || '';
    const apellidos = this.formularioDatosPersonales.get('APELLIDOS')!.value || '';

    this.formularioDatosPersonales.get('NOMBRE_PACIENTE')!.setValue(`${nombres} ${apellidos}`.trim(), { emitEvent: false });
  }

  async guardarDatosPersonales() {
    let historiaClinica = this.formularioDatosPersonales.value.IDANAMNESIS_TEXTO;
    let idAnamnesis = this.formularioDatosPersonales.value.IDANAMNESIS;
    let nombrePaciente = this.formularioDatosPersonales.value.NOMBRE_PACIENTE;

    if (!historiaClinica) {
      await this.mensajesUsuariosService.mensajeInformativo("NO SE PUEDE GUARDAR SIN HISTORIA CLINICA");
      return;
    }

    if (!nombrePaciente) {
      await this.mensajesUsuariosService.mensajeInformativo("NO SE PUEDE GUARDAR SIN NOMBRE DE PACIENTE");
      return;
    }

    let datosParaGurdarEnAnamnesis: RespuestaDatosPersonales = new RespuestaDatosPersonales();
    let datosFormularioGurdarEnAnamnesis = { ...this.formularioDatosPersonales.value };

    // 🔹 Convertir todos los valores de tipo string a mayúsculas
    Object.keys(datosFormularioGurdarEnAnamnesis).forEach(campo => {
      if (typeof datosFormularioGurdarEnAnamnesis[campo] === 'string') {
        datosFormularioGurdarEnAnamnesis[campo] = datosFormularioGurdarEnAnamnesis[campo].toUpperCase();
      }
    });

    datosParaGurdarEnAnamnesis.datosPersonales = datosFormularioGurdarEnAnamnesis;
    console.log(datosParaGurdarEnAnamnesis.datosPersonales);

    await this.respuestaPinService.updateNombrePacienteEscogidoData(datosFormularioGurdarEnAnamnesis.NOMBRE_PACIENTE);
    datosParaGurdarEnAnamnesis.strFotoFrontal = this.fotoFrontalBase64;

    if (idAnamnesis != 0) {
      // Si existe IDANAMNESIS, actualizar (editar)
      await this.agregarDatosPersonalesService.startConnectionEditarDatosPersonales(this.idSedeActualSignalR, JSON.stringify(datosParaGurdarEnAnamnesis));
      console.log('Editando datos personales...');
    } else {
      // Si no existe IDANAMNESIS, crear nuevo registro
      console.log('Guardando datos personales...');
      await this.agregarDatosPersonalesService.startConnectionGuardarDatosPersonales(this.idSedeActualSignalR, JSON.stringify(datosParaGurdarEnAnamnesis));
    }
  }
  ...
  private formatFechaToISO(fecha: string): string {
    if (!fecha) return '';

    const [dia, mes, anio] = fecha.split('/').map(Number);
    if (!dia || !mes || !anio) return '';

    const date = new Date(anio, mes - 1, dia); // Meses en JS van de 0 a 11
    return date.toISOString(); // Devuelve formato "YYYY-MM-DDTHH:mm:ss.sssZ"
  }
  ...
  convertirAHora24(hora12: string): string {
    ...
    if (ampm === 'PM' && horas24 < 12) {
      horas24 += 12; // Convierte a formato de 24 horas para PM
    }
    if (ampm === 'AM' && horas24 === 12) {
      horas24 = 0; // Convierte a 0 para medianoche
    }
  ...
  /*async guardarDatosPersonales() {
    ...
    if (historiaClinica && nombrePaciente) {
      if (imgFirmaPaciente) {
      ...
      if (this.idSedeActualSignalR != '') {
        ...
        let datosParaGuradarEnAnamnesis: RespuestaDatosPersonales = new RespuestaDatosPersonales();
        datosParaGuradarEnAnamnesis.datosPersonales.IDANAMNESIS_TEXTO = this.formularioDatosPersonales.value.IDANAMNESIS_TEXTO;
        datosParaGuradarEnAnamnesis.datosPersonales.IDANAMNESIS = this.formularioDatosPersonales.value.IDANAMNESIS;
        datosParaGuradarEnAnamnesis.datosPersonales.NOTA_IMPORTANTE = this.formularioDatosPersonales.value.NOTA_IMPORTANTE;
        datosParaGuradarEnAnamnesis.datosPersonales.COMPARACION = this.formularioDatosPersonales.value.COMPARACION;
        datosParaGuradarEnAnamnesis.datosPersonales.FECHA_INGRESO = this.formularioDatosPersonales.value.FECHA_INGRESO;
        datosParaGuradarEnAnamnesis.datosPersonales.FECHA_INGRESO_DATE = this.formularioDatosPersonales.value.FECHA_INGRESO_DATE;
        datosParaGuradarEnAnamnesis.datosPersonales.HORA_INGRESO = this.formularioDatosPersonales.value.HORA_INGRESO;
        datosParaGuradarEnAnamnesis.datosPersonales.NOMBRES = this.formularioDatosPersonales.value.NOMBRES;
        datosParaGuradarEnAnamnesis.datosPersonales.APELLIDOS = this.formularioDatosPersonales.value.APELLIDOS;
        datosParaGuradarEnAnamnesis.datosPersonales.NOMBRE_PACIENTE = this.formularioDatosPersonales.value.NOMBRE_PACIENTE;
        datosParaGuradarEnAnamnesis.datosPersonales.FECHAN_DIA = this.formularioDatosPersonales.value.FECHAN_DIA;
        datosParaGuradarEnAnamnesis.datosPersonales.FECHAN_MES = this.formularioDatosPersonales.value.FECHAN_MES;
        datosParaGuradarEnAnamnesis.datosPersonales.FECHAN_ANO = this.formularioDatosPersonales.value.FECHAN_ANO;
        datosParaGuradarEnAnamnesis.datosPersonales.DOCUMENTO_IDENTIDAD = this.formularioDatosPersonales.value.DOCUMENTO_IDENTIDAD;
        datosParaGuradarEnAnamnesis.datosPersonales.SEXO = this.formularioDatosPersonales.value.SEXO;
        datosParaGuradarEnAnamnesis.datosPersonales.EDAD = this.formularioDatosPersonales.value.EDAD;
        datosParaGuradarEnAnamnesis.datosPersonales.EDADMES = this.formularioDatosPersonales.value.EDADMES;
        datosParaGuradarEnAnamnesis.datosPersonales.DIRECCION_PACIENTE = this.formularioDatosPersonales.value.DIRECCION_PACIENTE;
        datosParaGuradarEnAnamnesis.datosPersonales.TELF_P = this.formularioDatosPersonales.value.TELF_P;
        datosParaGuradarEnAnamnesis.datosPersonales.TELF_P_OTRO = this.formularioDatosPersonales.value.TELF_P_OTRO;
        datosParaGuradarEnAnamnesis.datosPersonales.CELULAR_P = this.formularioDatosPersonales.value.CELULAR_P;
        datosParaGuradarEnAnamnesis.datosPersonales.NOMBRE_RESPONS = this.formularioDatosPersonales.value.NOMBRE_RESPONS;
        datosParaGuradarEnAnamnesis.datosPersonales.DIRECCION_RESPONSABLE = this.formularioDatosPersonales.value.DIRECCION_RESPONSABLE;
        datosParaGuradarEnAnamnesis.datosPersonales.TELF_RESP = this.formularioDatosPersonales.value.TELF_RESP;
        datosParaGuradarEnAnamnesis.datosPersonales.TELF_OF_RESP = this.formularioDatosPersonales.value.TELF_OF_RESP;
        datosParaGuradarEnAnamnesis.datosPersonales.CELULAR_RESPONSABLE = this.formularioDatosPersonales.value.CELULAR_RESPONSABLE;
        datosParaGuradarEnAnamnesis.datosPersonales.BEEPER_RESPONSABLE = this.formularioDatosPersonales.value.BEEPER_RESPONSABLE;
        datosParaGuradarEnAnamnesis.datosPersonales.COD_BEEPR_RESP = this.formularioDatosPersonales.value.COD_BEEPR_RESP;
        datosParaGuradarEnAnamnesis.datosPersonales.E_MAIL_RESP = this.formularioDatosPersonales.value.E_MAIL_RESP;
        datosParaGuradarEnAnamnesis.datosPersonales.REFERIDO_POR = this.formularioDatosPersonales.value.REFERIDO_POR;
        datosParaGuradarEnAnamnesis.datosPersonales.NRO_AFILIACION = this.formularioDatosPersonales.value.NRO_AFILIACION;
        datosParaGuradarEnAnamnesis.datosPersonales.CONVENIO = this.formularioDatosPersonales.value.CONVENIO;
        datosParaGuradarEnAnamnesis.datosPersonales.ESTADO_TRATAMIENTO = this.formularioDatosPersonales.value.ESTADO_TRATAMIENTO;
        datosParaGuradarEnAnamnesis.datosPersonales.TIPO_PACIENTE = this.formularioDatosPersonales.value.TIPO_PACIENTE;
        datosParaGuradarEnAnamnesis.datosPersonales.CEDULA_NUMERO = this.formularioDatosPersonales.value.CEDULA_NUMERO;
        datosParaGuradarEnAnamnesis.datosPersonales.ESTADOCIVIL = this.formularioDatosPersonales.value.ESTADOCIVIL;
        datosParaGuradarEnAnamnesis.datosPersonales.PARENTESCO = this.formularioDatosPersonales.value.PARENTESCO;
        datosParaGuradarEnAnamnesis.datosPersonales.NIVELESCOLAR = this.formularioDatosPersonales.value.NIVELESCOLAR;
        datosParaGuradarEnAnamnesis.datosPersonales.ZONA_RECIDENCIAL = this.formularioDatosPersonales.value.ZONA_RECIDENCIAL;
        datosParaGuradarEnAnamnesis.datosPersonales.PARENTESCO_RESPONSABLE = this.formularioDatosPersonales.value.PARENTESCO_RESPONSABLE;
        datosParaGuradarEnAnamnesis.datosPersonales.DOMICILIO = this.formularioDatosPersonales.value.DOMICILIO;
        datosParaGuradarEnAnamnesis.datosPersonales.EMERGENCIA = this.formularioDatosPersonales.value.EMERGENCIA;
        datosParaGuradarEnAnamnesis.datosPersonales.ACOMPANATE_TEL = this.formularioDatosPersonales.value.ACOMPANATE_TEL;
        datosParaGuradarEnAnamnesis.datosPersonales.ACOMPANATE = this.formularioDatosPersonales.value.ACOMPANATE;
        datosParaGuradarEnAnamnesis.datosPersonales.BARRIO = this.formularioDatosPersonales.value.BARRIO;
        datosParaGuradarEnAnamnesis.datosPersonales.LUGAR = this.formularioDatosPersonales.value.LUGAR;
        datosParaGuradarEnAnamnesis.datosPersonales.DOCUMENTO_RESPONS = this.formularioDatosPersonales.value.DOCUMENTO_RESPONS;
        datosParaGuradarEnAnamnesis.datosPersonales.ACTIVIDAD_ECONOMICA = this.formularioDatosPersonales.value.ACTIVIDAD_ECONOMICA;
        datosParaGuradarEnAnamnesis.datosPersonales.ESTRATO = this.formularioDatosPersonales.value.ESTRATO;
        datosParaGuradarEnAnamnesis.datosPersonales.LUGAR_NACIMIENTO = this.formularioDatosPersonales.value.LUGAR_NACIMIENTO;
        datosParaGuradarEnAnamnesis.datosPersonales.CODIGO_CIUDAD = this.formularioDatosPersonales.value.CODIGO_CIUDAD;
        datosParaGuradarEnAnamnesis.datosPersonales.CODIGO_DEPARTAMENTO = this.formularioDatosPersonales.value.CODIGO_DEPARTAMENTO;
        datosParaGuradarEnAnamnesis.datosPersonales.CODIGO_EPS = this.formularioDatosPersonales.value.CODIGO_EPS;
        datosParaGuradarEnAnamnesis.datosPersonales.CODIGO_EPS_LISTADO = this.formularioDatosPersonales.value.CODIGO_EPS_LISTADO;
        datosParaGuradarEnAnamnesis.datosPersonales.NUMERO_TTITULAR = this.formularioDatosPersonales.value.NUMERO_TTITULAR;
        datosParaGuradarEnAnamnesis.datosPersonales.NOMBREPADRE = this.formularioDatosPersonales.value.NOMBREPADRE;
        datosParaGuradarEnAnamnesis.datosPersonales.TELEFONOPADRE = this.formularioDatosPersonales.value.TELEFONOPADRE;
        datosParaGuradarEnAnamnesis.datosPersonales.NOMBRE_MADRE = this.formularioDatosPersonales.value.NOMBRE_MADRE;
        datosParaGuradarEnAnamnesis.datosPersonales.TELEFONOMADRE = this.formularioDatosPersonales.value.TELEFONOMADRE;
        datosParaGuradarEnAnamnesis.datosPersonales.CEL_PADRE = this.formularioDatosPersonales.value.CEL_PADRE;
        datosParaGuradarEnAnamnesis.datosPersonales.CEL_MADRE = this.formularioDatosPersonales.value.CEL_MADRE;
        datosParaGuradarEnAnamnesis.datosPersonales.OCUPACION_PADRE = this.formularioDatosPersonales.value.OCUPACION_PADRE;
        datosParaGuradarEnAnamnesis.datosPersonales.OCUPACION_MADRE = this.formularioDatosPersonales.value.OCUPACION_MADRE;
        datosParaGuradarEnAnamnesis.datosPersonales.NUMEROHERMANOS = this.formularioDatosPersonales.value.NUMEROHERMANOS;
        datosParaGuradarEnAnamnesis.datosPersonales.RELACIONPADRES = this.formularioDatosPersonales.value.RELACIONPADRES;
        datosParaGuradarEnAnamnesis.datosPersonales.ACTIVO = this.formularioDatosPersonales.value.ACTIVO;
        datosParaGuradarEnAnamnesis.datosPersonales.IDREFERIDOPOR = this.formularioDatosPersonales.value.IDREFERIDOPOR;
        datosParaGuradarEnAnamnesis.strFotoFrontal = this.fotoFrontalBase64;

        await this.agregarDatosPersonalesService.startConnectionGuardarDatosPersonales(this.idSedeActualSignalR, JSON.stringify(datosParaGuradarEnAnamnesis));
      }
    } else {
      await this.mensajesUsuariosService.mensajeInformativo("NO HAY DATOS PERSONALES PARA GUARDAR");
    }

  }*/

  cancelarDatosPersonales() {
    this.respuestaPinService.updatedatosPersonalesParaEditar(new RespuestaDatosPersonales());
  ...
  // Sube la foto desde el almacenamiento
  ...
  private convertirABase64(file: File): void {
    ...
    reader.onload = () => {
      this.fotoFrontalBase64 = reader.result as string;
    };
    reader.readAsDataURL(file); // Convierte el archivo a Base64
  }

  // Activa el clic en el input de la cámara
  activarCamara(): void {
    this.inputCamara.nativeElement.click(); // Usamos nativeElement
  }

  // Activa el clic en el input de subida
  activarSubida(): void {
    this.inputFile.nativeElement.click(); // Usamos nativeElement
  }
