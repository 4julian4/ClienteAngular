...
export class AgregarDatosPersonalesComponent implements OnInit {
  ...
  @Input() resultadoBusquedaDatosPersonalesCompletos: DatosPersonales =
    new DatosPersonales();
  ...
  resultadoBusquedaDatosPersonalesCompletosPorCambioDeDoctor: RespuestaDatosPersonales =
    new RespuestaDatosPersonales();
  mensajeNotaImportante: string = '';
  notaImportante: string = '';
  idAnamnesisParaGuardarEditar: number = 0;
  //---------emsayo covenios----------------------//
  // ✅ Lista real de convenios (la que ya traes: 20 o 4000)
  listaConvenios: ListadoItem[] = [];

  // ✅ Control del buscador del select
  convenioFiltroCtrl = new FormControl<string>('', { nonNullable: true });

  // ✅ Lista filtrada (lo que se pinta en el select)
  conveniosFiltrados$ = new ReplaySubject<ListadoItem[]>(1);
  //-----------------------------------------------//
  ...
  async ngOnInit(): Promise<void> {
    ...
    this.formularioDatosPersonales.valueChanges
      .pipe(takeUntil(this.destruir$))
      .subscribe((valores) => {
        Object.keys(valores).forEach((campo) => {
          if (typeof valores[campo] === 'string') {
            ...
            this.formularioDatosPersonales
              .get(campo)
              ?.setValue(valores[campo].toUpperCase(), { emitEvent: false });
          }
        });
      });

    // ✅ Cuando el usuario escribe en el buscador, filtramos
    this.convenioFiltroCtrl.valueChanges
      .pipe(takeUntil(this.destruir$))
      .subscribe(() => this.filtrarConvenios());
    ...
    this.formularioDatosPersonales
      ...
      .subscribe(() => this.actualizarNombreCompleto());
    this.formularioDatosPersonales
      .get('APELLIDOS')!
      .valueChanges.pipe(takeUntil(this.destruir$))
      .subscribe(() => this.actualizarNombreCompleto());
    ...
    this.respuestaPinService.shareddatosRespuestaPinData
      ...
      .subscribe((data) => {
        if (data != null) {
          ...
          // ✅ AQUI: asigna tu lista real de convenios
          // OJO: cambia "lstConvenios" por el nombre real que te llega
          this.listaConvenios = data.lstConvenios ?? [];
          this.conveniosFiltrados$.next(this.listaConvenios.slice());
        }
      });
    ...
    //Datos Personales
    this.datosPersonalesService.respuestaDatosPersonalesEmit
      .pipe(takeUntil(this.destruir$))
      .subscribe(
    ...
    if (this.formularioDatosPersonales) {
      this.formularioDatosPersonales
        .get('CODIGO_EPS_LISTADO')
        ?.valueChanges.subscribe((nombre) => {
          const eps = this.listaEps.find((eps) => eps.NOMBRE === nombre);
          if (eps) {
            this.formularioDatosPersonales
              .get('CODIGO_EPS')
              ?.setValue(eps.CODIGO, { emitEvent: false });
          }
        });

      this.formularioDatosPersonales
        .get('CODIGO_EPS')
        ?.valueChanges.subscribe((codigo) => {
          const eps = this.listaEps.find((eps) => eps.CODIGO === codigo);
          if (eps) {
            this.formularioDatosPersonales
              .get('CODIGO_EPS_LISTADO')
              ?.setValue(eps.NOMBRE, { emitEvent: false });
          }
        });
    }
  ...
  actualizarNombreCompleto(): void {
    ...
    const apellidos =
      this.formularioDatosPersonales.get('APELLIDOS')!.value || '';

    this.formularioDatosPersonales
      .get('NOMBRE_PACIENTE')!
      .setValue(`${nombres} ${apellidos}`.trim(), { emitEvent: false });
  }

  private filtrarConvenios(): void {
    const term = (this.convenioFiltroCtrl.value || '').toLowerCase().trim();
    ...
    this.conveniosFiltrados$.next(
      this.listaConvenios.filter((c) =>
  ...
  async guardarDatosPersonales() {
    let historiaClinica =
      this.formularioDatosPersonales.value.IDANAMNESIS_TEXTO;
    ...
    if (!historiaClinica) {
      await this.mensajesUsuariosService.mensajeInformativo(
        'NO SE PUEDE GUARDAR SIN HISTORIA CLINICA'
      );
      return;
    }
    ...
    if (!nombrePaciente) {
      await this.mensajesUsuariosService.mensajeInformativo(
        'NO SE PUEDE GUARDAR SIN NOMBRE DE PACIENTE'
      );
      return;
    }

    let datosParaGurdarEnAnamnesis: RespuestaDatosPersonales =
      new RespuestaDatosPersonales();
    let datosFormularioGurdarEnAnamnesis = {
      ...this.formularioDatosPersonales.value,
    };
    ...
    Object.keys(datosFormularioGurdarEnAnamnesis).forEach((campo) => {
      if (typeof datosFormularioGurdarEnAnamnesis[campo] === 'string') {
        datosFormularioGurdarEnAnamnesis[campo] =
          datosFormularioGurdarEnAnamnesis[campo].toUpperCase();
      }
    });

    datosParaGurdarEnAnamnesis.datosPersonales =
      datosFormularioGurdarEnAnamnesis;
    ...
    if (idAnamnesis != 0) {
      ...
      await this.agregarDatosPersonalesService.startConnectionEditarDatosPersonales(
        this.idSedeActualSignalR,
        JSON.stringify(datosParaGurdarEnAnamnesis)
      );
      console.log('Editando datos personales...');
    } else {
      // Si no existe IDANAMNESIS, crear nuevo registro
      console.log('Guardando datos personales...');
      await this.agregarDatosPersonalesService.startConnectionGuardarDatosPersonales(
        this.idSedeActualSignalR,
        JSON.stringify(datosParaGurdarEnAnamnesis)
      );
    }
