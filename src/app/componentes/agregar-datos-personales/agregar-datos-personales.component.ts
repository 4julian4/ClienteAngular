import {
  Component,
  ElementRef,
  Input,
  OnDestroy,
  OnInit,
  ViewChild,
} from '@angular/core';
import { FormBuilder, FormControl, FormGroup } from '@angular/forms';
import {
  DatosPersonales,
  DatosPersonalesService,
} from 'src/app/conexiones/rydent/modelos/datos-personales';
import { RespuestaDatosPersonales } from 'src/app/conexiones/rydent/modelos/respuesta-datos-personales';
import {
  ListadoItem,
  RespuestaPinService,
} from 'src/app/conexiones/rydent/modelos/respuesta-pin';
import { CodigosCiudades } from 'src/app/conexiones/rydent/tablas/codigos-ciudades';
import { CodigosDepartamentos } from 'src/app/conexiones/rydent/tablas/codigos-departamentos';
import { CodigosEps } from 'src/app/conexiones/rydent/tablas/codigos-eps';
import { AgregarDatosPersonalesService } from './agregar-datos-personales.service';
import { MensajesUsuariosService } from '../mensajes-usuarios';
import { Router } from '@angular/router';
import { ReplaySubject, Subject, takeUntil } from 'rxjs';

@Component({
  selector: 'app-agregar-datos-personales',
  templateUrl: './agregar-datos-personales.component.html',
  styleUrls: ['./agregar-datos-personales.component.scss'],
})
export class AgregarDatosPersonalesComponent implements OnInit, OnDestroy {
  @ViewChild('inputCamara', { static: false }) inputCamara!: ElementRef;
  @ViewChild('inputFile', { static: false }) inputFile!: ElementRef;
  @Input() resultadoBusquedaDatosPersonalesCompletos: DatosPersonales =
    new DatosPersonales();
  formularioDatosPersonales!: FormGroup;
  idSedeActualSignalR: string = '';
  listaEps: CodigosEps[] = [];
  listaDepartamentos: CodigosDepartamentos[] = [];
  // ✅ esta será la lista TOTAL que llega del PIN
  listaCiudadesTodas: CodigosCiudades[] = [];

  // ✅ esta es la lista que se pinta en el select
  listaCiudadesFiltradas: CodigosCiudades[] = [];
  listaCiudades: CodigosCiudades[] = [];
  idAnamnesisPacienteSeleccionado: number = 0;
  fotoFrontalBase64: string = '';
  doctorGuardar: string = '';
  idDoctorGuardar: number = 0;
  resultadoBusquedaDatosPersonalesCompletosPorCambioDeDoctor: RespuestaDatosPersonales =
    new RespuestaDatosPersonales();
  mensajeNotaImportante: string = '';
  notaImportante: string = '';
  idAnamnesisParaGuardarEditar: number = 0;

  sedeIdSeleccionada = 0;

  //---------emsayo covenios----------------------//
  // ✅ Lista real de convenios (la que ya traes: 20 o 4000)
  listaConvenios: ListadoItem[] = [];

  // ✅ Control del buscador del select
  convenioFiltroCtrl = new FormControl<string>('', { nonNullable: true });

  // ✅ Lista filtrada (lo que se pinta en el select)
  conveniosFiltrados$ = new ReplaySubject<ListadoItem[]>(1);
  //-----------------------------------------------//

  private destruir$ = new Subject<void>();
  //fotoFrontalBase64: string = ''; // Aquí se almacena la imagen en base64
  imagenPorDefecto: string =
    'data:image/png;base64,';
  formularioDeshabilitado: boolean = false;

  // ✅ FIX: mapa de meses (para FECHAN_MES VARCHAR(4))
  private readonly MESES_MAP: Record<string, string> = {
    ENE: 'ENE',
    FEB: 'FEB',
    MAR: 'MAR',
    ABR: 'ABR',
    MAY: 'MAY',
    JUN: 'JUN',
    JUL: 'JUL',
    AGO: 'AGO',
    SEP: 'SEP',
    OCT: 'OCT',
    NOV: 'NOV',
    DIC: 'DIC',
  };

  // ✅ FIX: límites reales de tu tabla TANAMNESIS (solo los que nos están reventando + los más críticos)
  private readonly DB_LIMITS: Record<string, number> = {
    IDANAMNESIS_TEXTO: 25,
    FECHA_INGRESO: 30,
    NOMBRES: 150,
    APELLIDOS: 150,
    NOMBRE_PACIENTE: 50,
    FECHAN_DIA: 2,
    FECHAN_MES: 4,
    FECHAN_ANO: 5,
    DOCUMENTO_IDENTIDAD: 7,
    SEXO: 12,
    EDAD: 20,
    EDADMES: 20,
    DIRECCION_PACIENTE: 100,
    TELF_P: 50,
    TELF_P_OTRO: 50,
    CELULAR_P: 100,
    NOMBRE_RESPONS: 50,
    DIRECCION_RESPONSABLE: 100,
    TELF_RESP: 50,
    TELF_OF_RESP: 50,
    CELULAR_RESPONSABLE: 100,
    E_MAIL_RESP: 60,
    REFERIDO_POR: 255,
    CEDULA_NUMERO: 25,
    ESTADOCIVIL: 50,
    NIVELESCOLAR: 50,
    ZONA_RECIDENCIAL: 20,
    PARENTESCO_RESPONSABLE: 40,
    DOMICILIO: 250,
    EMERGENCIA: 200,
    ACOMPANATE: 100,
    ACOMPANATE_TEL: 25,
    BARRIO: 80,
    LUGAR: 100,
    ACTIVIDAD_ECONOMICA: 60,
    ESTRATO: 100,
    LUGAR_NACIMIENTO: 100,
    CODIGO_EPS: 6,
    CODIGO_EPS_LISTADO: 6, // 🔥 este era el que te explotaba guardando NOMBRE
    CODIGO_CIUDAD: 3, // 🔥
    CODIGO_DEPARTAMENTO: 3, // 🔥
    TIPO_PACIENTE: 70,
    ESTADO_TRATAMIENTO: 70,
    CONVENIO: 100,
    NOTA_IMPORTANTE: 200,
    IMPORTANTE: 500,
    DOCUMENTO_RESPONS: 50,
  };

  constructor(
    private formBuilder: FormBuilder,
    private datosPersonalesService: DatosPersonalesService,
    private respuestaPinService: RespuestaPinService,
    private agregarDatosPersonalesService: AgregarDatosPersonalesService,
    private mensajesUsuariosService: MensajesUsuariosService,
    private router: Router,
  ) {}

  async ngOnInit(): Promise<void> {
    this.respuestaPinService.sharedidDoctorSeleccionadoData
      .pipe(takeUntil(this.destruir$))
      .subscribe((data) => {
        if (data != null) {
          this.idDoctorGuardar = data;
        }
      });

    this.respuestaPinService.sharedcambiarDoctorSeleccionadoData
      .pipe(takeUntil(this.destruir$))
      .subscribe((data) => {
        if (data != null) {
          this.doctorGuardar = data;
        }
      });

    this.inicializarFormulario();

    this.formularioDatosPersonales.valueChanges
      .pipe(takeUntil(this.destruir$))
      .subscribe((valores) => {
        Object.keys(valores).forEach((campo) => {
          if (typeof valores[campo] === 'string') {
            this.formularioDatosPersonales
              .get(campo)
              ?.setValue(valores[campo].toUpperCase(), { emitEvent: false });
          }
        });
      });

    // ✅ Cuando el usuario escribe en el buscador, filtramos
    this.convenioFiltroCtrl.valueChanges
      .pipe(takeUntil(this.destruir$))
      .subscribe(() => this.filtrarConvenios());

    this.respuestaPinService.shareddatosPersonalesParaEditarData
      .pipe(takeUntil(this.destruir$))
      .subscribe((data) => {
        if (!data || !data.datosPersonales) {
          return;
        }

        this.resultadoBusquedaDatosPersonalesCompletos = data.datosPersonales;
        this.respuestaPinService.updateNotaImportante(
          this.resultadoBusquedaDatosPersonalesCompletos.IMPORTANTE,
        );
        this.fotoFrontalBase64 = data.strFotoFrontal || this.imagenPorDefecto;

        const horaFormateada = this.convertirAHora24(
          this.resultadoBusquedaDatosPersonalesCompletos.HORA_INGRESO.toString(),
        );

        this.formularioDatosPersonales.patchValue({
          ...this.resultadoBusquedaDatosPersonalesCompletos,
          HORA_INGRESO: horaFormateada,
        });

        this.recalcularEdadDesdeFechaNacimiento();
      });

    this.respuestaPinService.shareddatosPersonalesParaCrearDesdeInteroperabilidadData
      .pipe(takeUntil(this.destruir$))
      .subscribe((data) => {
        if (!data || !data.datosPersonales) return;

        const datos = data.datosPersonales;

        this.resultadoBusquedaDatosPersonalesCompletos = datos;
        this.fotoFrontalBase64 = data.strFotoFrontal || this.imagenPorDefecto;

        let horaFormateada = '';
        if (datos.HORA_INGRESO) {
          horaFormateada = this.convertirAHora24(String(datos.HORA_INGRESO));
        }

        this.formularioDatosPersonales.patchValue({
          ...datos,
          IDANAMNESIS: 0,
          HORA_INGRESO:
            horaFormateada ||
            this.formularioDatosPersonales.get('HORA_INGRESO')?.value,
        });

        this.recalcularEdadDesdeFechaNacimiento();
      });

    this.formularioDatosPersonales
      .get('CODIGO_DEPARTAMENTO')
      ?.valueChanges.pipe(takeUntil(this.destruir$))
      .subscribe((codigoDepto) => {
        // 1) Filtra lista de ciudades
        this.filtrarCiudadesPorDepartamento(codigoDepto);

        // 2) Limpia ciudad (porque cambió el depto)
        this.formularioDatosPersonales
          .get('CODIGO_CIUDAD')
          ?.setValue('', { emitEvent: false });
      });

    // Suscribirse a cambios en los campos "NOMBRES" y "APELLIDOS"
    this.formularioDatosPersonales
      .get('NOMBRES')!
      .valueChanges.pipe(takeUntil(this.destruir$))
      .subscribe(() => this.actualizarNombreCompleto());
    this.formularioDatosPersonales
      .get('APELLIDOS')!
      .valueChanges.pipe(takeUntil(this.destruir$))
      .subscribe(() => this.actualizarNombreCompleto());

    // ✅ FIX: cada vez que cambie fecha nacimiento, recalcular EDAD/EDADMES
    this.formularioDatosPersonales
      .get('FECHAN_DIA')!
      .valueChanges.pipe(takeUntil(this.destruir$))
      .subscribe(() => this.recalcularEdadDesdeFechaNacimiento());
    this.formularioDatosPersonales
      .get('FECHAN_MES')!
      .valueChanges.pipe(takeUntil(this.destruir$))
      .subscribe(() => this.recalcularEdadDesdeFechaNacimiento());
    this.formularioDatosPersonales
      .get('FECHAN_ANO')!
      .valueChanges.pipe(takeUntil(this.destruir$))
      .subscribe(() => this.recalcularEdadDesdeFechaNacimiento());

    this.respuestaPinService.shareddatosRespuestaPinData
      .pipe(takeUntil(this.destruir$))
      .subscribe((data) => {
        if (data != null) {
          this.listaEps = data.lstEps;
          this.listaDepartamentos = data.lstDepartamentos;
          //this.listaCiudades = data.lstCiudades;
          this.listaCiudadesTodas = data.lstCiudades ?? [];
          console.log(this.listaCiudadesTodas);
          console.log(this.listaDepartamentos);
          console.log(this.listaEps);
          this.filtrarCiudadesPorDepartamento(
            this.formularioDatosPersonales.get('CODIGO_DEPARTAMENTO')?.value,
          );

          this.listaConvenios = data.lstConvenios ?? [];
          this.conveniosFiltrados$.next(this.listaConvenios.slice());
        }
      });

    this.respuestaPinService.sharedSedeSeleccionada
      .pipe(takeUntil(this.destruir$))
      .subscribe((id) => {
        this.sedeIdSeleccionada = id ?? 0;
      });

    this.respuestaPinService.sharedSedeData
      .pipe(takeUntil(this.destruir$))
      .subscribe((data) => {
        if (data != null) {
          this.idSedeActualSignalR = data;
        }
      });

    //Datos Personales
    this.datosPersonalesService.respuestaDatosPersonalesEmit
      .pipe(takeUntil(this.destruir$))
      .subscribe(
        async (respuestaBusquedaDatosPersonales: RespuestaDatosPersonales) => {
          this.resultadoBusquedaDatosPersonalesCompletos =
            respuestaBusquedaDatosPersonales.datosPersonales;
          this.respuestaPinService.updateNotaImportante(
            this.resultadoBusquedaDatosPersonalesCompletos.IMPORTANTE,
          );

          this.fotoFrontalBase64 =
            respuestaBusquedaDatosPersonales.strFotoFrontal;
          if (this.fotoFrontalBase64 == null || this.fotoFrontalBase64 == '') {
            this.fotoFrontalBase64 = this.imagenPorDefecto;
          }

          this.formularioDatosPersonales.patchValue(
            this.resultadoBusquedaDatosPersonalesCompletos,
          );

          // ✅ FIX
          this.recalcularEdadDesdeFechaNacimiento();
        },
      );

    // ✅ IMPORTANTE:
    // Ya NO sincronizamos EPS por "nombre", porque el nombre explota VARCHAR(6).
    // En el HTML corregido, el select guardará CODIGO en CODIGO_EPS_LISTADO (6).
    // Así que dejamos este bloque sin daño: si existe, solo nos aseguramos que ambos queden con código.
    if (this.formularioDatosPersonales) {
      this.formularioDatosPersonales
        .get('CODIGO_EPS_LISTADO')
        ?.valueChanges.pipe(takeUntil(this.destruir$))
        .subscribe((codigo) => {
          const eps = this.listaEps.find((eps) => eps.CODIGO === codigo);
          if (eps) {
            this.formularioDatosPersonales
              .get('CODIGO_EPS')
              ?.setValue(eps.CODIGO, { emitEvent: false });
          }
        });

      this.formularioDatosPersonales
        .get('CODIGO_EPS')
        ?.valueChanges.pipe(takeUntil(this.destruir$))
        .subscribe((codigo) => {
          const eps = this.listaEps.find((eps) => eps.CODIGO === codigo);
          if (eps) {
            this.formularioDatosPersonales
              .get('CODIGO_EPS_LISTADO')
              ?.setValue(eps.CODIGO, { emitEvent: false });
          }
        });
    }
  }

  private formatHora(hora: any): string {
    if (!hora) return '';

    let date: Date;

    if (hora instanceof Date) {
      date = hora;
    } else if (typeof hora === 'string') {
      const [horas, minutos, segundos] = hora.split(':').map(Number);
      if (isNaN(horas) || isNaN(minutos)) return '';
      date = new Date();
      date.setHours(horas, minutos, segundos || 0);
    } else {
      return '';
    }

    if (isNaN(date.getTime())) return '';

    let horas = date.getHours();
    let minutos = date.getMinutes();
    const ampm = horas >= 12 ? 'PM' : 'AM';

    horas = horas % 12;
    horas = horas ? horas : 12;
    const minutosStr = minutos < 10 ? '0' + minutos : minutos;

    return `${horas}:${minutosStr} ${ampm}`;
  }

  private filtrarCiudadesPorDepartamento(codigoDepto: any) {
    const depto = String(codigoDepto ?? '').trim();

    if (!depto) {
      // si no hay depto seleccionado, puedes mostrar 0 ciudades o todas
      this.listaCiudadesFiltradas = [];
      return;
    }

    // ⚠️ Ajusta el nombre del campo según tu modelo real:
    // A veces es CODIGO_DEPARTAMENTO, COD_DEP, DEPTO, etc.
    this.listaCiudadesFiltradas = (this.listaCiudadesTodas ?? []).filter(
      (c) => String((c as any).CODIGO_DEPARTAMENTO ?? '').trim() === depto,
    );
  }

  inicializarFormulario() {
    console.log(this.doctorGuardar);
    console.log(this.idDoctorGuardar);
    const fechaActual = new Date();
    const horaActual = fechaActual.getHours() + ':' + fechaActual.getMinutes();

    this.formularioDatosPersonales = this.formBuilder.group({
      IDANAMNESIS: ['0'],
      IDANAMNESIS_TEXTO: [''],
      NOTA_IMPORTANTE: [''],
      COMPARACION: [0],
      FECHA_INGRESO: [fechaActual],
      FECHA_INGRESO_DATE: [fechaActual],
      HORA_INGRESO: [this.convertirAHora24(horaActual)],
      NOMBRES: [''],
      APELLIDOS: [''],
      NOMBRE_PACIENTE: [''],
      FECHAN_DIA: [''],
      FECHAN_MES: [''],
      FECHAN_ANO: [''],
      DOCUMENTO_IDENTIDAD: [''],
      SEXO: [''],
      EDAD: [''],
      EDADMES: [''],
      DIRECCION_PACIENTE: [''],
      TELF_P: [''],
      TELF_P_OTRO: [''],
      CELULAR_P: [''],
      NOMBRE_RESPONS: [''],
      DIRECCION_RESPONSABLE: [''],
      TELF_RESP: [''],
      TELF_OF_RESP: [''],
      CELULAR_RESPONSABLE: [''],
      BEEPER_RESPONSABLE: ['0'],
      COD_BEEPR_RESP: ['0'],
      E_MAIL_RESP: [''],
      REFERIDO_POR: [''],
      NRO_AFILIACION: [''],
      CONVENIO: [''],
      ESTADO_TRATAMIENTO: [''],
      TIPO_PACIENTE: [''],
      CEDULA_NUMERO: [''],
      ESTADOCIVIL: [''],
      PARENTESCO: [''],
      NIVELESCOLAR: [''],
      ZONA_RECIDENCIAL: [''],
      PARENTESCO_RESPONSABLE: [''],
      DOMICILIO: [''],
      EMERGENCIA: [''],
      ACOMPANATE_TEL: [''],
      ACOMPANATE: [''],
      BARRIO: [''],
      LUGAR: [''],
      DOCUMENTO_RESPONS: [''],
      ACTIVIDAD_ECONOMICA: [''],
      ESTRATO: [''],
      LUGAR_NACIMIENTO: [''],
      CODIGO_CIUDAD: [''],
      CODIGO_DEPARTAMENTO: [''],
      CODIGO_EPS: [''],
      CODIGO_EPS_LISTADO: [''],
      NUMERO_TTITULAR: [0],
      NOMBREPADRE: [''],
      TELEFONOPADRE: [''],
      NOMBRE_MADRE: [''],
      TELEFONOMADRE: [''],
      CEL_PADRE: [''],
      CEL_MADRE: [''],
      OCUPACION_PADRE: [''],
      OCUPACION_MADRE: [''],
      NUMEROHERMANOS: [''],
      RELACIONPADRES: [''],
      ACTIVO: ['0'],
      IDREFERIDOPOR: ['0'],
      COD_DOCTOR: [this.idDoctorGuardar],
      DOCTOR: [this.doctorGuardar],
    });
  }

  actualizarNombreCompleto(): void {
    const nombres = this.formularioDatosPersonales.get('NOMBRES')!.value || '';
    const apellidos =
      this.formularioDatosPersonales.get('APELLIDOS')!.value || '';

    this.formularioDatosPersonales
      .get('NOMBRE_PACIENTE')!
      .setValue(`${nombres} ${apellidos}`.trim(), { emitEvent: false });
  }

  private filtrarConvenios(): void {
    const term = (this.convenioFiltroCtrl.value || '').toLowerCase().trim();

    if (!term) {
      this.conveniosFiltrados$.next(this.listaConvenios.slice());
      return;
    }

    this.conveniosFiltrados$.next(
      this.listaConvenios.filter((c) =>
        (c?.nombre ?? '').toLowerCase().includes(term),
      ),
    );
  }

  // ✅ FIX: recorta string a tamaño máximo (evita truncation)
  private clampStr(value: any, max: number): any {
    if (value === null || value === undefined) return value;
    if (typeof value !== 'string') return value;
    const v = value.trim();
    if (!max || max <= 0) return v;
    return v.length > max ? v.substring(0, max) : v;
  }

  // ✅ FIX: normaliza mes (Ene/Feb/etc) y lo deja en 3 letras max 4 (según BD)
  private normalizarMes(mes: any): string {
    if (!mes) return '';
    const m = String(mes).trim().toUpperCase();
    const key = m.substring(0, 3);
    return this.MESES_MAP[key] ?? this.clampStr(m, 4);
  }

  // ✅ FIX: calcula EDAD y EDADMES a partir de FECHAN_DIA/MES/AÑO
  private recalcularEdadDesdeFechaNacimiento(): void {
    const dia = String(
      this.formularioDatosPersonales.get('FECHAN_DIA')?.value || '',
    ).trim();
    const mes = String(
      this.formularioDatosPersonales.get('FECHAN_MES')?.value || '',
    ).trim();
    const anio = String(
      this.formularioDatosPersonales.get('FECHAN_ANO')?.value || '',
    ).trim();

    const diaNum = parseInt(dia, 10);
    const anioNum = parseInt(anio, 10);
    const mesKey = this.normalizarMes(mes).substring(0, 3);

    const mapMesNum: Record<string, number> = {
      ENE: 1,
      FEB: 2,
      MAR: 3,
      ABR: 4,
      MAY: 5,
      JUN: 6,
      JUL: 7,
      AGO: 8,
      SEP: 9,
      OCT: 10,
      NOV: 11,
      DIC: 12,
    };
    const mesNum = mapMesNum[mesKey];

    if (!diaNum || !mesNum || !anioNum || anioNum < 1900 || anioNum > 2100) {
      return; // no dañamos nada si está incompleto
    }

    const hoy = new Date();
    const fechaNac = new Date(anioNum, mesNum - 1, diaNum);

    if (isNaN(fechaNac.getTime())) return;

    let years = hoy.getFullYear() - fechaNac.getFullYear();
    let months = hoy.getMonth() - fechaNac.getMonth();

    if (hoy.getDate() < fechaNac.getDate()) {
      months -= 1;
    }
    if (months < 0) {
      years -= 1;
      months += 12;
    }
    if (years < 0) years = 0;
    if (months < 0) months = 0;

    this.formularioDatosPersonales
      .get('EDAD')
      ?.setValue(String(years), { emitEvent: false });
    this.formularioDatosPersonales
      .get('EDADMES')
      ?.setValue(String(months), { emitEvent: false });

    // ✅ normaliza el mes al guardable
    this.formularioDatosPersonales
      .get('FECHAN_MES')
      ?.setValue(this.normalizarMes(mes), { emitEvent: false });
  }

  // ✅ FIX: normaliza TODA la data antes de enviar a Firebird (evita truncation y tipos)
  private normalizarParaDb(payload: any): any {
    const obj = { ...payload };

    // 1) Strings: uppercase + recorte por tamaño real BD
    Object.keys(obj).forEach((campo) => {
      if (typeof obj[campo] === 'string') {
        obj[campo] = obj[campo].toUpperCase();
      }
      if (this.DB_LIMITS[campo]) {
        obj[campo] = this.clampStr(obj[campo], this.DB_LIMITS[campo]);
      }
    });

    // 2) Mes: FECHAN_MES VARCHAR(4)
    obj.FECHAN_MES = this.normalizarMes(obj.FECHAN_MES);

    // 3) NOMBRE_PACIENTE siempre recortado a 50 (BD)
    obj.NOMBRE_PACIENTE = this.clampStr(obj.NOMBRE_PACIENTE, 50);

    // 4) Campos código (NO NOMBRE) -> ya viene desde HTML corregido
    obj.CODIGO_EPS = this.clampStr(obj.CODIGO_EPS, 6);
    obj.CODIGO_EPS_LISTADO = this.clampStr(obj.CODIGO_EPS_LISTADO, 6);
    obj.CODIGO_CIUDAD = this.clampStr(obj.CODIGO_CIUDAD, 3);
    obj.CODIGO_DEPARTAMENTO = this.clampStr(obj.CODIGO_DEPARTAMENTO, 3);

    // 5) Números (evita overflow raro por strings)
    const toInt = (v: any, def: number = 0) => {
      const n = parseInt(String(v ?? '').trim(), 10);
      return isNaN(n) ? def : n;
    };

    obj.IDANAMNESIS = toInt(obj.IDANAMNESIS, 0);
    obj.COMPARACION = obj.COMPARACION ?? 0;
    obj.BEEPER_RESPONSABLE = toInt(obj.BEEPER_RESPONSABLE, 0);
    obj.COD_BEEPR_RESP = toInt(obj.COD_BEEPR_RESP, 0);
    obj.NUMERO_TTITULAR = toInt(obj.NUMERO_TTITULAR, 0);
    obj.IDREFERIDOPOR = toInt(obj.IDREFERIDOPOR, 0);
    obj.COD_DOCTOR = toInt(obj.COD_DOCTOR, this.idDoctorGuardar);

    // 6) ACTIVO NOT NULL (BD)
    obj.ACTIVO = toInt(obj.ACTIVO, 0);

    // 7) Seguridad extra: si algo viene undefined, lo dejamos en '' para strings
    // (Firebird a veces se queja si el provider envía null de forma rara)
    Object.keys(this.DB_LIMITS).forEach((campo) => {
      if (obj[campo] === undefined || obj[campo] === null) {
        // solo para campos string típicos
        if (typeof obj[campo] === 'string') obj[campo] = '';
      }
    });

    return obj;
  }

  async guardarDatosPersonales() {
    let historiaClinica =
      this.formularioDatosPersonales.value.IDANAMNESIS_TEXTO;
    let idAnamnesis = this.formularioDatosPersonales.value.IDANAMNESIS;
    let nombrePaciente = this.formularioDatosPersonales.value.NOMBRE_PACIENTE;

    if (!historiaClinica) {
      await this.mensajesUsuariosService.mensajeInformativo(
        'NO SE PUEDE GUARDAR SIN HISTORIA CLINICA',
      );
      return;
    }

    if (!nombrePaciente) {
      await this.mensajesUsuariosService.mensajeInformativo(
        'NO SE PUEDE GUARDAR SIN NOMBRE DE PACIENTE',
      );
      return;
    }

    // ✅ FIX: recalcula edad justo antes de enviar
    this.recalcularEdadDesdeFechaNacimiento();

    let datosParaGurdarEnAnamnesis: RespuestaDatosPersonales =
      new RespuestaDatosPersonales();
    let datosFormularioGurdarEnAnamnesis = {
      ...this.formularioDatosPersonales.value,
    };

    // 🔹 Convertir todos los valores de tipo string a mayúsculas
    Object.keys(datosFormularioGurdarEnAnamnesis).forEach((campo) => {
      if (typeof datosFormularioGurdarEnAnamnesis[campo] === 'string') {
        datosFormularioGurdarEnAnamnesis[campo] =
          datosFormularioGurdarEnAnamnesis[campo].toUpperCase();
      }
    });

    // ✅ FIX: NORMALIZA FINAL (recorta a tamaños BD + códigos correctos)
    datosFormularioGurdarEnAnamnesis = this.normalizarParaDb(
      datosFormularioGurdarEnAnamnesis,
    );

    datosParaGurdarEnAnamnesis.datosPersonales =
      datosFormularioGurdarEnAnamnesis;

    console.log(datosParaGurdarEnAnamnesis.datosPersonales);

    await this.respuestaPinService.updateNombrePacienteEscogidoData(
      datosFormularioGurdarEnAnamnesis.NOMBRE_PACIENTE,
    );
    datosParaGurdarEnAnamnesis.strFotoFrontal = this.fotoFrontalBase64;

    if (idAnamnesis != 0) {
      await this.agregarDatosPersonalesService.startConnectionEditarDatosPersonales(
        this.sedeIdSeleccionada,
        JSON.stringify(datosParaGurdarEnAnamnesis),
      );
      console.log('Editando datos personales...');
    } else {
      console.log('Guardando datos personales...');
      await this.agregarDatosPersonalesService.startConnectionGuardarDatosPersonales(
        this.sedeIdSeleccionada,
        JSON.stringify(datosParaGurdarEnAnamnesis),
      );
    }
  }

  ngOnDestroy(): void {
    this.destruir$.next();
    this.destruir$.complete();
  }

  private formatFechaToISO(fecha: string): string {
    if (!fecha) return '';

    const [dia, mes, anio] = fecha.split('/').map(Number);
    if (!dia || !mes || !anio) return '';

    const date = new Date(anio, mes - 1, dia);
    return date.toISOString();
  }

  convertirAHora24(hora12: string): string {
    const [hora, minutosYampm] = hora12.split(':');
    const [minutos, ampm] = minutosYampm.split(' ');

    let horas24 = parseInt(hora);
    if (ampm === 'PM' && horas24 < 12) {
      horas24 += 12;
    }
    if (ampm === 'AM' && horas24 === 12) {
      horas24 = 0;
    }

    return `${horas24.toString().padStart(2, '0')}:${minutos}`;
  }

  cancelarDatosPersonales() {
    this.respuestaPinService.updatedatosPersonalesParaEditar(
      new RespuestaDatosPersonales(),
    );

    this.respuestaPinService.updatedatosPersonalesParaCrearDesdeInteroperabilidad(
      new RespuestaDatosPersonales(),
    );

    this.router.navigate(['/datos-personales']);
  }

  async mostrarAntecedentes() {}

  async mostrarEvolucion() {}

  // Captura la foto desde la cámara
  capturarFoto(event: Event): void {
    const input = event.target as HTMLInputElement;
    if (input.files && input.files.length > 0) {
      const file = input.files[0];
      this.convertirABase64(file);
    }
  }

  // Sube la foto desde el almacenamiento
  subirFoto(event: Event): void {
    const input = event.target as HTMLInputElement;
    if (input.files && input.files.length > 0) {
      const file = input.files[0];
      this.convertirABase64(file);
    }
  }

  // Convierte la imagen seleccionada a Base64
  private convertirABase64(file: File): void {
    const reader = new FileReader();
    reader.onload = () => {
      this.fotoFrontalBase64 = reader.result as string;
    };
    reader.readAsDataURL(file);
  }

  // Activa el clic en el input de la cámara
  activarCamara(): void {
    this.inputCamara.nativeElement.click();
  }

  // Activa el clic en el input de subida
  activarSubida(): void {
    this.inputFile.nativeElement.click();
  }
}
