import { Component, Input, OnDestroy, OnInit } from '@angular/core';
import { FormBuilder, FormGroup } from '@angular/forms';
import { Router } from '@angular/router';
import { Subject, takeUntil } from 'rxjs';
import {
  DatosPersonales,
  DatosPersonalesService,
} from 'src/app/conexiones/rydent/modelos/datos-personales';
import { RespuestaDatosPersonales } from 'src/app/conexiones/rydent/modelos/respuesta-datos-personales';
import { RespuestaPinService } from 'src/app/conexiones/rydent/modelos/respuesta-pin';
import { CodigosCiudades } from 'src/app/conexiones/rydent/tablas/codigos-ciudades';
import { CodigosDepartamentos } from 'src/app/conexiones/rydent/tablas/codigos-departamentos';
import { CodigosEps } from 'src/app/conexiones/rydent/tablas/codigos-eps';

@Component({
  selector: 'app-datos-personales',
  templateUrl: './datos-personales.component.html',
  styleUrl: './datos-personales.component.scss',
})
export class DatosPersonalesComponent implements OnInit, OnDestroy {
  @Input() resultadoBusquedaDatosPersonalesCompletos: DatosPersonales =
    new DatosPersonales();

  formularioDatosPersonales!: FormGroup;
  idSedeActualSignalR: string = '';
  listaEps: CodigosEps[] = [];
  listaDepartamentos: CodigosDepartamentos[] = [];
  listaCiudades: CodigosCiudades[] = [];
  idAnamnesisPacienteSeleccionado: number = 0;
  fotoFrontalBase64: string = '';
  resultadoBusquedaDatosPersonalesCompletosPorCambioDeDoctor: RespuestaDatosPersonales =
    new RespuestaDatosPersonales();
  mensajeNotaImportante: string = '';
  notaImportante: string = '';
  imagenPorDefecto: string =
    'data:image/png;base64,';
  formularioDeshabilitado: boolean = false;
  sedeIdSeleccionada = 0;
  private destruir$ = new Subject<void>();

  constructor(
    private formBuilder: FormBuilder,
    private datosPersonalesService: DatosPersonalesService,
    private respuestaPinService: RespuestaPinService,
    private router: Router,
  ) {}

  // ======================================================
  // ✅ Helpers robustos (porque EPS/DEP/CIUDAD no comparten el mismo nombre de campo)
  // ======================================================
  private getEpsCode(e: any): string {
    return (e?.CODIGO ?? e?.codigo ?? '').toString();
  }

  private getDepCode(d: any): string {
    return (d?.CODIGO_DEPARTAMENTO ?? d?.CODIGO ?? d?.codigo ?? '').toString();
  }

  private getCityCode(c: any): string {
    return (c?.CODIGO_CIUDAD ?? c?.CODIGO ?? c?.codigo ?? '').toString();
  }

  async ngOnInit(): Promise<void> {
    this.inicializarFormulario();

    // ======================================================
    // Subs: anamnesis
    // ======================================================
    this.respuestaPinService.sharedAnamnesisData
      .pipe(takeUntil(this.destruir$))
      .subscribe((data) => {
        if (data != null) {
          this.idAnamnesisPacienteSeleccionado = data;
          console.log('ID ANAMNESIS: ' + this.idAnamnesisPacienteSeleccionado);
        }
      });

    // ======================================================
    // Subs: catálogos (EPS / Departamentos / Ciudades)
    // ======================================================
    this.respuestaPinService.shareddatosRespuestaPinData
      .pipe(takeUntil(this.destruir$))
      .subscribe((data) => {
        if (data != null) {
          this.listaEps = data.lstEps;
          this.listaDepartamentos = data.lstDepartamentos;
          this.listaCiudades = data.lstCiudades;
          console.log('Catálogos recibidos: ', {
            eps: this.listaEps,
            departamentos: this.listaDepartamentos,
            ciudades: this.listaCiudades,
          });

          // ✅ Fix visual cuando los catálogos llegan después del patchValue
          this.fixListadosSiVienenComoCodigo();
          this.syncListadosDesdeCodigos();
        }
      });

    // ======================================================
    // Subs: sede
    // ======================================================
    this.respuestaPinService.sharedSedeSeleccionada
      .pipe(takeUntil(this.destruir$))
      .subscribe((id) => {
        this.sedeIdSeleccionada = id ?? 0;
      });

    this.respuestaPinService.sharedSedeData
      .pipe(takeUntil(this.destruir$))
      .subscribe((data) => {
        if (data != null) {
          this.idSedeActualSignalR = data;
        }
      });

    // ======================================================
    // Pide datos del paciente
    // ======================================================
    await this.obtenerDatosCompletosPaciente(
      this.idAnamnesisPacienteSeleccionado,
    );

    // ======================================================
    // Respuesta datos personales
    // ======================================================
    this.datosPersonalesService.respuestaDatosPersonalesEmit
      .pipe(takeUntil(this.destruir$))
      .subscribe(
        async (respuestaBusquedaDatosPersonales: RespuestaDatosPersonales) => {
          this.resultadoBusquedaDatosPersonalesCompletos =
            respuestaBusquedaDatosPersonales.datosPersonales;

          console.log(
            'Datos Personales Completos: ',
            this.resultadoBusquedaDatosPersonalesCompletos,
          );

          this.respuestaPinService.updateNotaImportante(
            this.resultadoBusquedaDatosPersonalesCompletos.IMPORTANTE,
          );

          this.fotoFrontalBase64 =
            respuestaBusquedaDatosPersonales.strFotoFrontal;

          const fechaFormateada = this.formatFecha(
            this.resultadoBusquedaDatosPersonalesCompletos.FECHA_INGRESO,
          );
          const horaFormateada = this.formatHora(
            this.resultadoBusquedaDatosPersonalesCompletos.HORA_INGRESO,
          );

          if (this.fotoFrontalBase64 == null || this.fotoFrontalBase64 == '') {
            this.fotoFrontalBase64 = this.imagenPorDefecto;
          }

          // ✅ Patch sin disparar valueChanges (evita loops y parpadeos)
          this.formularioDatosPersonales.patchValue(
            {
              ...this.resultadoBusquedaDatosPersonalesCompletos,
              FECHA_INGRESO: fechaFormateada,
              HORA_INGRESO: horaFormateada,
            },
            { emitEvent: false },
          );

          // ✅ Fix inmediato si el backend manda LISTADO como código
          this.fixListadosSiVienenComoCodigo();

          // ✅ Si ya hay código y ya hay catálogo, fuerza LISTADO=NOMBRE
          this.syncListadosDesdeCodigos();
        },
      );

    // ======================================================
    // ValueChanges: EPS / DEP / CIUDAD
    // ======================================================
    this.configurarValueChangesListados();
  }

  ngOnDestroy(): void {
    this.destruir$.next();
    this.destruir$.complete();
  }

  async obtenerDatosCompletosPaciente(idAnamnesis: number) {
    if (this.idSedeActualSignalR != '') {
      await this.datosPersonalesService.startConnectionRespuestaDatosPersonales(
        this.sedeIdSeleccionada,
        idAnamnesis.toString(),
      );
    }
  }

  inicializarFormulario() {
    this.formularioDatosPersonales = this.formBuilder.group({
      IDANAMNESIS: [''],
      IDANAMNESIS_TEXTO: [''],
      NOTA_IMPORTANTE: [''],
      COMPARACION: [0],
      FECHA_INGRESO: [''],
      FECHA_INGRESO_DATE: [new Date()],
      HORA_INGRESO: [new Date()],
      NOMBRES: [''],
      APELLIDOS: [''],
      NOMBRE_PACIENTE: [''],
      FECHAN_DIA: [''],
      FECHAN_MES: [''],
      FECHAN_ANO: [''],
      DOCUMENTO_IDENTIDAD: [''],
      SEXO: [''],
      EDAD: [''],
      EDADMES: [''],
      DIRECCION_PACIENTE: [''],
      TELF_P: [''],
      TELF_P_OTRO: [''],
      CELULAR_P: [''],
      NOMBRE_RESPONS: [''],
      DIRECCION_RESPONSABLE: [''],
      TELF_RESP: [''],
      TELF_OF_RESP: [''],
      CELULAR_RESPONSABLE: [''],
      BEEPER_RESPONSABLE: ['0'],
      COD_BEEPR_RESP: ['0'],
      E_MAIL_RESP: [''],
      REFERIDO_POR: [''],
      NRO_AFILIACION: [''],
      CONVENIO: [''],
      ESTADO_TRATAMIENTO: [''],
      TIPO_PACIENTE: [''],
      CEDULA_NUMERO: [''],
      ESTADOCIVIL: [''],
      PARENTESCO: [''],
      NIVELESCOLAR: [''],
      ZONA_RECIDENCIAL: [''],
      PARENTESCO_RESPONSABLE: [''],
      DOMICILIO: [''],
      EMERGENCIA: [''],
      ACOMPANATE_TEL: [''],
      ACOMPANATE: [''],
      BARRIO: [''],
      LUGAR: [''],
      DOCUMENTO_RESPONS: [''],
      ACTIVIDAD_ECONOMICA: [''],
      ESTRATO: [''],
      LUGAR_NACIMIENTO: [''],
      CODIGO_CIUDAD: [''],
      CODIGO_DEPARTAMENTO: [''],
      CODIGO_EPS: [''],

      // ✅ LISTADOS (visuales)
      CODIGO_EPS_LISTADO: [''],
      CODIGO_DEPARTAMENTO_LISTADO: [''],
      CODIGO_CIUDAD_LISTADO: [''],

      NUMERO_TTITULAR: [''],
      NOMBREPADRE: [''],
      TELEFONOPADRE: [''],
      NOMBRE_MADRE: [''],
      TELEFONOMADRE: [''],
      CEL_PADRE: [''],
      CEL_MADRE: [''],
      OCUPACION_PADRE: [''],
      OCUPACION_MADRE: [''],
      NUMEROHERMANOS: [''],
      RELACIONPADRES: [''],
      ACTIVO: ['0'],
      IDREFERIDOPOR: ['0'],
      COD_DOCTOR: ['0'],
      DOCTOR: [''],
    });
  }

  guardarDatosPersonales() {}
  cancelarDatosPersonales() {}

  async editarPaciente() {
    let datosPersonales = this.formularioDatosPersonales.getRawValue();
    let fotoFrontal = this.fotoFrontalBase64;

    console.log('Datos del paciente para editar:', datosPersonales);

    datosPersonales.FECHA_INGRESO = this.formatFechaToISO(
      datosPersonales.FECHA_INGRESO,
    );
    datosPersonales.HORA_INGRESO = this.formatHoraTo24H(
      datosPersonales.HORA_INGRESO,
    );
    console.log('2. FORM YA TRANSFORMADO:', datosPersonales);

    console.log('Datos del paciente para editar:', datosPersonales);

    let datosPersonalesCompletos = new RespuestaDatosPersonales();
    datosPersonalesCompletos.datosPersonales = datosPersonales;
    datosPersonalesCompletos.strFotoFrontal = fotoFrontal;
    console.log('3. OBJETO FINAL ENVIADO A EDITAR:', datosPersonalesCompletos);

    await this.respuestaPinService.updatedatosPersonalesParaEditar(
      datosPersonalesCompletos,
    );
    console.log(
      '4. VALOR ACTUAL EN SERVICE ANTES DE NAVEGAR:',
      this.respuestaPinService['datosPersonalesParaEditar']?.getValue?.(),
    );
    this.router.navigate(['/agregar-datos-personales']);
    console.log('Datos del paciente para editar:', datosPersonales);
  }

  // ======================================================
  // ✅ LISTADOS: valueChanges
  // ======================================================
  private configurarValueChangesListados(): void {
    if (!this.formularioDatosPersonales) return;

    // ---------- EPS ----------
    this.formularioDatosPersonales
      .get('CODIGO_EPS_LISTADO')
      ?.valueChanges.pipe(takeUntil(this.destruir$))
      .subscribe((valor) => {
        if (!valor) return;

        // acepta que venga NOMBRE o CODIGO
        const eps = this.listaEps.find(
          (e: any) => e?.NOMBRE === valor || this.getEpsCode(e) === valor,
        );
        if (eps) {
          const epsCode = this.getEpsCode(eps);

          this.formularioDatosPersonales
            .get('CODIGO_EPS')
            ?.setValue(epsCode, { emitEvent: false });

          this.formularioDatosPersonales
            .get('CODIGO_EPS_LISTADO')
            ?.setValue(eps.NOMBRE, { emitEvent: false });
        }
      });

    this.formularioDatosPersonales
      .get('CODIGO_EPS')
      ?.valueChanges.pipe(takeUntil(this.destruir$))
      .subscribe((codigo) => {
        if (!codigo) return;

        const eps = this.listaEps.find(
          (e: any) => this.getEpsCode(e) === codigo,
        );
        if (eps) {
          this.formularioDatosPersonales
            .get('CODIGO_EPS_LISTADO')
            ?.setValue(eps.NOMBRE, { emitEvent: false });
        }
      });

    // ---------- DEPARTAMENTO ----------
    this.formularioDatosPersonales
      .get('CODIGO_DEPARTAMENTO_LISTADO')
      ?.valueChanges.pipe(takeUntil(this.destruir$))
      .subscribe((valor) => {
        if (!valor) return;

        const dep = this.listaDepartamentos.find(
          (d: any) => d?.NOMBRE === valor || this.getDepCode(d) === valor,
        );

        if (dep) {
          const depCode = this.getDepCode(dep);

          this.formularioDatosPersonales
            .get('CODIGO_DEPARTAMENTO')
            ?.setValue(depCode, { emitEvent: false });

          this.formularioDatosPersonales
            .get('CODIGO_DEPARTAMENTO_LISTADO')
            ?.setValue(dep.NOMBRE, { emitEvent: false });

          // ✅ si cambia departamento, limpia ciudad (porque ya no coincide)
          this.formularioDatosPersonales
            .get('CODIGO_CIUDAD')
            ?.setValue('', { emitEvent: false });
          this.formularioDatosPersonales
            .get('CODIGO_CIUDAD_LISTADO')
            ?.setValue('', { emitEvent: false });
        }
      });

    this.formularioDatosPersonales
      .get('CODIGO_DEPARTAMENTO')
      ?.valueChanges.pipe(takeUntil(this.destruir$))
      .subscribe((codigo) => {
        if (!codigo) return;

        const dep = this.listaDepartamentos.find(
          (d: any) => this.getDepCode(d) === codigo,
        );
        if (dep) {
          this.formularioDatosPersonales
            .get('CODIGO_DEPARTAMENTO_LISTADO')
            ?.setValue(dep.NOMBRE, { emitEvent: false });
        }
      });

    // ---------- CIUDAD ----------
    this.formularioDatosPersonales
      .get('CODIGO_CIUDAD_LISTADO')
      ?.valueChanges.pipe(takeUntil(this.destruir$))
      .subscribe((valor) => {
        if (!valor) return;

        const ciudad = this.listaCiudades.find(
          (c: any) => c?.NOMBRE === valor || this.getCityCode(c) === valor,
        );

        if (ciudad) {
          const cityCode = this.getCityCode(ciudad);

          this.formularioDatosPersonales
            .get('CODIGO_CIUDAD')
            ?.setValue(cityCode, { emitEvent: false });

          this.formularioDatosPersonales
            .get('CODIGO_CIUDAD_LISTADO')
            ?.setValue(ciudad.NOMBRE, { emitEvent: false });
        }
      });

    this.formularioDatosPersonales
      .get('CODIGO_CIUDAD')
      ?.valueChanges.pipe(takeUntil(this.destruir$))
      .subscribe((codigo) => {
        if (!codigo) return;

        const ciudad = this.listaCiudades.find(
          (c: any) => this.getCityCode(c) === codigo,
        );
        if (ciudad) {
          this.formularioDatosPersonales
            .get('CODIGO_CIUDAD_LISTADO')
            ?.setValue(ciudad.NOMBRE, { emitEvent: false });
        }
      });
  }

  // ======================================================
  // ✅ FIX: cuando backend manda LISTADO = CODIGO
  // ======================================================
  private fixListadosSiVienenComoCodigo(): void {
    if (!this.formularioDatosPersonales) return;

    // EPS
    const epsListado =
      this.formularioDatosPersonales.get('CODIGO_EPS_LISTADO')?.value;
    if (epsListado && this.listaEps?.length) {
      const eps = this.listaEps.find(
        (e: any) => this.getEpsCode(e) === epsListado,
      );
      if (eps) {
        const epsCode = this.getEpsCode(eps);

        this.formularioDatosPersonales
          .get('CODIGO_EPS')
          ?.setValue(epsCode, { emitEvent: false });

        this.formularioDatosPersonales
          .get('CODIGO_EPS_LISTADO')
          ?.setValue(eps.NOMBRE, { emitEvent: false });
      }
    }

    // DEP
    const depListado = this.formularioDatosPersonales.get(
      'CODIGO_DEPARTAMENTO_LISTADO',
    )?.value;

    if (depListado && this.listaDepartamentos?.length) {
      const dep = this.listaDepartamentos.find(
        (d: any) => this.getDepCode(d) === depListado,
      );
      if (dep) {
        const depCode = this.getDepCode(dep);

        this.formularioDatosPersonales
          .get('CODIGO_DEPARTAMENTO')
          ?.setValue(depCode, { emitEvent: false });

        this.formularioDatosPersonales
          .get('CODIGO_DEPARTAMENTO_LISTADO')
          ?.setValue(dep.NOMBRE, { emitEvent: false });
      }
    }

    // CIUDAD
    const ciudadListado = this.formularioDatosPersonales.get(
      'CODIGO_CIUDAD_LISTADO',
    )?.value;

    if (ciudadListado && this.listaCiudades?.length) {
      const ciudad = this.listaCiudades.find(
        (c: any) => this.getCityCode(c) === ciudadListado,
      );
      if (ciudad) {
        const cityCode = this.getCityCode(ciudad);

        this.formularioDatosPersonales
          .get('CODIGO_CIUDAD')
          ?.setValue(cityCode, { emitEvent: false });

        this.formularioDatosPersonales
          .get('CODIGO_CIUDAD_LISTADO')
          ?.setValue(ciudad.NOMBRE, { emitEvent: false });
      }
    }
  }

  // ======================================================
  // ✅ Sync: si ya hay CÓDIGO, forzar LISTADO=NOMBRE
  // ======================================================
  private syncListadosDesdeCodigos(): void {
    if (!this.formularioDatosPersonales) return;

    // EPS
    const codEps = this.formularioDatosPersonales.get('CODIGO_EPS')?.value;
    if (codEps && this.listaEps?.length) {
      const eps = this.listaEps.find((e: any) => this.getEpsCode(e) === codEps);
      if (eps) {
        this.formularioDatosPersonales
          .get('CODIGO_EPS_LISTADO')
          ?.setValue(eps.NOMBRE, { emitEvent: false });
      }
    }

    // DEP
    const codDep = this.formularioDatosPersonales.get(
      'CODIGO_DEPARTAMENTO',
    )?.value;
    if (codDep && this.listaDepartamentos?.length) {
      const dep = this.listaDepartamentos.find(
        (d: any) => this.getDepCode(d) === codDep,
      );
      if (dep) {
        this.formularioDatosPersonales
          .get('CODIGO_DEPARTAMENTO_LISTADO')
          ?.setValue(dep.NOMBRE, { emitEvent: false });
      }
    }

    // CIUDAD
    const codCiudad =
      this.formularioDatosPersonales.get('CODIGO_CIUDAD')?.value;
    if (codCiudad && this.listaCiudades?.length) {
      const ciudad = this.listaCiudades.find(
        (c: any) => this.getCityCode(c) === codCiudad,
      );
      if (ciudad) {
        this.formularioDatosPersonales
          .get('CODIGO_CIUDAD_LISTADO')
          ?.setValue(ciudad.NOMBRE, { emitEvent: false });
      }
    }
  }

  // ======================================================
  // Utils fechas / horas (tuyas igual)
  // ======================================================
  private formatFecha(fecha: string): string {
    if (!fecha) return '';
    const date = new Date(fecha);
    if (isNaN(date.getTime())) return '';
    const dia = date.getDate().toString().padStart(2, '0');
    const mes = (date.getMonth() + 1).toString().padStart(2, '0');
    const anio = date.getFullYear();
    return `${dia}/${mes}/${anio}`;
  }

  private formatHora(hora: any): string {
    if (!hora) return '';
    let date: Date;

    if (hora instanceof Date) {
      date = hora;
    } else if (typeof hora === 'string') {
      const [horas, minutos, segundos] = hora.split(':').map(Number);
      if (isNaN(horas) || isNaN(minutos)) return '';
      date = new Date();
      date.setHours(horas, minutos, segundos || 0);
    } else {
      return '';
    }

    if (isNaN(date.getTime())) return '';

    let horas = date.getHours();
    let minutos = date.getMinutes();
    const ampm = horas >= 12 ? 'PM' : 'AM';

    horas = horas % 12;
    horas = horas ? horas : 12;
    const minutosStr = minutos < 10 ? '0' + minutos : minutos;

    return `${horas}:${minutosStr} ${ampm}`;
  }

  private formatFechaToISO(fecha: string): string {
    if (!fecha) return '';
    const [dia, mes, anio] = fecha.split('/').map(Number);
    if (!dia || !mes || !anio) return '';
    const date = new Date(anio, mes - 1, dia);
    return date.toISOString();
  }

  private formatHoraTo24H(hora: string): string {
    if (!hora) return '';
    const match = hora.match(/(\d+):(\d+) (AM|PM)/);
    if (!match) return '';
    let horas = parseInt(match[1], 10);
    const minutos = match[2];
    const ampm = match[3];
    if (ampm === 'PM' && horas < 12) horas += 12;
    if (ampm === 'AM' && horas === 12) horas = 0;
    return `${horas.toString().padStart(2, '0')}:${minutos}:00`;
  }

  async mostrarAntecedentes() {}
  async mostrarEvolucion() {}
}
