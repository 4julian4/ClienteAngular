...
export class EvolucionComponent implements OnInit, OnDestroy {
  @Input() respuestaEvolucionPaciente: RespuestaEvolucionPaciente =
  ...
  async ngOnInit(): Promise<void> {
    ...
    this.respuestaEvolucionPacienteService.respuestaEvolucionPacienteEmit
      .pipe(takeUntil(this.destruir$))
      .subscribe(
        async (respuestaBusquedaEvolucion: RespuestaEvolucionPaciente[]) => {
          ...
          if (this.resultadoBusquedaEvolucion.length > 0) {
            this.resultadoBusquedaEvolucion.sort((b, a) => {
              if (a.evolucion.FECHA && b.evolucion.FECHA) {
                return (
                  new Date(a.evolucion.FECHA).getTime() -
                  new Date(b.evolucion.FECHA).getTime()
                );
              } else {
                return 0;
              }
