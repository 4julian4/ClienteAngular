...
export class ClientesService {
  ...
  public GetAll(): Observable<Clientes[]> {
    console.log(urlPage + ' - ' + environment.apiUrl);

    return this.httpClient.get<Clientes[]>(urlPage, environment.httpOptions);
  }
  /*public GetAll(): Observable<Clientes[]> {
    const url = 'https://localhost:7028/api/Clientes'; // <-- directo a la API
    console.log('GET ->', url);
    return this.httpClient.get<Clientes[]>(url, environment.httpOptions);
  }*/

  public Edit(_Clientes: Clientes): Observable<boolean> {
    return this.httpClient.put<boolean>(
      urlPage + '/' + _Clientes.idCliente,
      _Clientes,
