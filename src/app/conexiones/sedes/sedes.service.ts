...
export class SedesService {
  ...
  public async ConsultarSedePorIdentificadorLocal(
    identificadorLocal: string,
  ): Promise<Sedes> {
    let url =
      urlPage + '/ConsultarSedePorIdentificadorLocal/' + identificadorLocal;
  ...
  public async ConsultarPorIdCliente(idCliente: string): Promise<Sedes[]> {
    ...
    const url = urlPage + '/ConsultarPorIdCliente/' + idCliente;
    console.log(
      'URL SEDES ConsultarPorIdCliente:',
      url,
      'apiUrl:',
      environment.apiUrl,
    );

    const obj = await this.httpClient.get<Sedes[]>(
      url,
